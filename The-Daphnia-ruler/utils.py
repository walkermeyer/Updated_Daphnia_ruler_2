...
def make_res(img, props, scf, image, eyeMethod = False, tail_Length = None, head_Length = None, daphnia_Length = None, angle = None):
    '''
...
def find_tip(binary1, cX, cY):
    ...
    new_lengths = lengths
    new_lengths[max_dist_index] = 0
    new_max = new_lengths.index(max(new_lengths))
    new_lengths[new_max] = 0

    max_dist_index2 = new_lengths.index(max(new_lengths))
    far_x2 = int(cX+dys[max_dist_index2])
    far_y2 = int(cY+dxs[max_dist_index2])
    daphnia_Length_eye_head_tip = max(new_lengths) 

    return([far_x, far_y, daphnia_Length_eye_tip, far_x2, far_y2, daphnia_Length_eye_head_tip])
...
def plt_head(img, far_x2, far_y2, cX, cY):
    '''
    plots tail on image.
    '''
    # plot tail on image
    cv2.line(img, (far_x2,far_y2),(cX, cY), (255, 0, 0), 2)
